...
export const reorderCollectionGames = async (collectionId, gameOrders) => {
  try {
    const serverAddress = await window.store.get("serverAddress");
    const token = await window.store.get("userToken");

    const response = await fetchWithConnectionTracking(
      buildServerUrl(serverAddress, `/api/collections/${collectionId}/games/reorder`),
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to reorder games');
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.error("[API] Error reordering games:", error);
    throw error;
  }
};
...
export const togglePinCollection = async (collectionId) => {
  try {
    const serverAddress = await window.store.get("serverAddress");
    const token = await window.store.get("userToken");

    const response = await fetchWithConnectionTracking(
      buildServerUrl(serverAddress, `/api/collections/${collectionId}/toggle-pin`),
      {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`
    ...
    console.error("[API] Error toggling pin:", error);
    throw error;
  }
};
...
export const getInstalledGames = async () => {
  try {
    ...
    const response = await fetchWithConnectionTracking(
      buildServerUrl(serverAddress, '/api/collections/smart/installed'),
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
    ...
    if (!response.ok) {
      throw new Error(`Error fetching installed games: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.debug("[API] Installed games unavailable");
    return null;
  }
};

export const getNotInstalledGames = async () => {
  try {
    const serverAddress = await window.store.get("serverAddress");
    const token = await window.store.get("userToken");

    const response = await fetchWithConnectionTracking(
      buildServerUrl(serverAddress, '/api/collections/smart/not-installed'),
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
    ...
    if (!response.ok) {
      throw new Error(`Error fetching not installed games: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.debug("[API] Not installed games unavailable");
    return null;
  }
};
...
export const getRecentlyPlayed = async () => {
  try {
    ...
    const response = await fetchWithConnectionTracking(
      buildServerUrl(serverAddress, '/api/collections/smart/recent'),
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
    ...
    if (!response.ok) {
      throw new Error(`Error fetching recently played games: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.debug("[API] Recently played games unavailable");
    return null;
  }
};
...
export const getMostPlayed = async () => {
  try {
    ...
    const response = await fetchWithConnectionTracking(
      buildServerUrl(serverAddress, '/api/collections/smart/most-played'),
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
    ...
    if (!response.ok) {
      throw new Error(`Error fetching most played games: ${response.status}`);
    }

    const data = await response.json();
    return data;
  } catch (error) {
    console.debug("[API] Most played games unavailable");
    return null;
  }
};
